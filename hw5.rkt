  ...
  (cond [(var? e) 
        ...
        [(call? e)
         ...
         (if (closure? v1) 
             (let* (
                  [fn (closure-fun v1)]
                  [cls-env (closure-env v1)]
                  [body (fun-body fn)]
                  [name (fun-nameopt fn)]
                  [args (fun-formal fn)]
                  [new-env (cons (cons args v2) cls-env)])
              (eval-under-env body (if name (cons (cons name v1) new-env) new-env)))
             (error "MUPL call apllied to non-closure")))]
        [(apair? e) (apair (eval-under-env (apair-e1 e) env) (eval-under-env (apair-e2 e) env))]
        [(fst? e)
         (let ([v1 (eval-under-env (fst-e e) env)])
           (if (apair? v1) (apair-e1 v1) (error "MUPL fst applied to non-pair")))]
        [(snd? e)
         (let ([v1 (eval-under-env (snd-e e) env)])
           (if (apair? v1) (apair-e2 v1) (error "MUPL snd applied to non-pair")))]
        [(isaunit? e)
...
(define mupl-map 
  (fun #f "f"
       (fun "list" "i" 
            (ifaunit (var "i") 
                     (aunit) 
                     (apair (call (var "f") (fst (var "i"))) 
                            (call (var "list") (snd (var "i"))))))))
