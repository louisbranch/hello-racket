...
(define tests
  (test-suite
   ...
   (check-equal? (string-append-map (list "") "1") '("1") "string-append-map test 2")

   ; list-nth-mod test
   (check-equal? (list-nth-mod (list 0 1 2 3 4) 2) 2 "list-nth-mod test")
   (check-equal? (list-nth-mod (list 0 1) 2) 0 "list-nth-mod test 2")
   (check-equal? (list-nth-mod (list 0 1 2) 4) 1 "list-nth-mod test 3")
   ...
   (check-equal? (stream-for-n-steps (lambda () (cons 1 ones)) 0) (list) "stream-for-n-steps test 3")
   ...
   (check-equal? (stream-for-n-steps funny-number-stream 1) (list 1) "funny-number-stream test 2")
   ...
   (check-equal? (stream-for-n-steps (stream-add-zero funny-number-stream) 5) 
                 (list (cons 0 1) (cons 0 2) (cons 0 3) (cons 0 4) (cons 0 -5)) "stream-add-zero test 2")
   ...
   (check-equal? (stream-for-n-steps (cycle-lists (list 1) (list "a")) 3) (list (cons 1 "a") (cons 1 "a") (cons 1 "a"))
                 "cycle-lists test 2")
   (check-equal? (stream-for-n-steps (cycle-lists (list 1) (list "a" "b")) 2) (list (cons 1 "a") (cons 1 "b"))
                 "cycle-lists test 3")

   ; vector-assoc test
   (check-equal? (vector-assoc 4 (vector (cons 2 1) (cons 3 1) (cons 4 1) (cons 5 1))) (cons 4 1) "vector-assoc test")
   (check-equal? (vector-assoc 4 (vector (cons 2 1) (cons 3 1) 4 (cons 4 1))) (cons 4 1) "vector-assoc test 2")
   (check-equal? (vector-assoc 4 (vector (cons 2 1) (cons 3 1) 4 (cons 5 1))) #f "vector-assoc test 3")
